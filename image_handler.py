...
from background_changer import set_background
...
def get_background_path(artist: str, album: str) -> str:
...
def get_full_background_path(background_path: str, filename: str) -> str:
	# returns the file path: ./ "downloaded_covers"/ artist/ album/ imagefilename.ext
	return (os.path.join(background_path, filename))
...
def get_cover(url: str, artist: str, album: str, size: int, ext: str = ".png", sep: str = " - ") -> None:
	...
	path = get_background_path(artist=artist, album=album)
	full_path = get_full_background_path(background_path=path, filename=cover_name)
	...
	set_background(background_path=full_path)
	return
